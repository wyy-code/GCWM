...
def _top_right_basis_from_gram(
    delta,
    rank=32,
    ...
    """
    Faster approximation of top right singular basis via randomized low-rank SVD.

    Returns:
      V_r: [d_in, r]   top right singular vectors
      S_r: [r]         top singular values
      fro_norm: scalar Frobenius norm of delta
    """
    # 为了速度，这里建议直接用 float32 做 randomized SVD
    # 后续几何步骤如果需要 float64，再在后面转回去
    x = delta.to(dtype=torch.float32)

    d_in = x.shape[1]
    fro_norm = torch.norm(x, p="fro").to(torch.float64)

    # q 是实际求的低秩维度；要略大于 rank
    q = min(rank + oversample, min(x.shape))
    if q <= 0:
        V_r = torch.zeros((d_in, 1), device=x.device, dtype=torch.float64)
        V_r[0, 0] = 1.0
        S_r = torch.ones((1,), device=x.device, dtype=torch.float64) * (reg ** 0.5)
        return V_r, S_r, fro_norm

    # randomized low-rank SVD
    U, S, V = torch.svd_lowrank(x, q=q, niter=niter)

    # torch.svd_lowrank 返回:
    # U: [d_out, q]
    # S: [q]
    # V: [d_in, q]
    # 其中 V 就是我们要的右奇异向量基
    ...
    # 和原实现对齐：用 evals = sigma^2 来做 energy truncation
    evals = S ** 2
...
def _bures_distance_sq(A, B, neg_tol=1e-12, zero_rel_tol=1e-10):
    A = _sym(A.to(dtype=torch.float64))
    B = _sym(B.to(dtype=torch.float64))

    A_sqrt = _matrix_sqrt_psd(A, neg_tol=neg_tol, zero_rel_tol=zero_rel_tol)
...
def _bures_distance_sq_dev64(A, B, neg_tol=1e-12, zero_rel_tol=1e-10):
    """
    Stable Bures distance on the current device in float64.
    Returns (clamped_value, raw_value_before_clamp).
    """
    dev = A.device
    A_dev = _sym(A.detach().to(dev, dtype=torch.float64))
    B_dev = _sym(B.detach().to(dev, dtype=torch.float64))

    A_sqrt = _matrix_sqrt_psd(A_dev, neg_tol=neg_tol, zero_rel_tol=zero_rel_tol)
    inner = _sym(A_sqrt @ B_dev @ A_sqrt)
    inner_sqrt = _matrix_sqrt_psd(inner, neg_tol=neg_tol, zero_rel_tol=zero_rel_tol)

    raw = torch.trace(A_dev) + torch.trace(B_dev) - 2.0 * torch.trace(inner_sqrt)
    d2 = torch.clamp(raw, min=0.0)
    return d2, raw
...
class SummaryStore:
    ...
    def _compute_layer_summary(self, delta, layer_key):
        ...
        summary = {
            ...
            "basis": V_r.detach().cpu(),
            "svals": S_r.detach().cpu(),
        }
        return summary
...
def _pair_metrics_from_cache(delta_i_layer, delta_j_layer, info_i, info_j, cov_reg=1e-6, bures_backend="gpu"):
    ...
    if bures_backend == "cpu":
        d2, raw = _bures_distance_sq_cpu64(B_i, B_j, neg_tol=1e-12, zero_rel_tol=1e-10)
    else:
        d2, raw = _bures_distance_sq_dev64(B_i, B_j, neg_tol=1e-12, zero_rel_tol=1e-10)
...
def _global_geometry_summary_from_cache(object_names, layer_infos, target_keys, cov_reg=1e-6, save_full_tensors=False, bures_backend="gpu"):
    layer_summaries = {}
    full_tensors = {}
    ...
    for key in tqdm(target_keys, desc="global-geometry", leave=False):
        ...
        for i, j in combinations(range(len(object_names)), 2):
            ...
            if bures_backend == "cpu":
                ...
                d2, raw = _bures_distance_sq_dev64(A, B, neg_tol=1e-12, zero_rel_tol=1e-10)
...
def run_bridge_analysis(
    ...
    shared_delta_cache = os.environ.get("BRIDGE_SHARED_DELTA_CACHE")
    cache_dir = shared_delta_cache if shared_delta_cache else os.path.join(output_dir, "delta_cache")
    delta_store = DeltaStore(base_model, cache_dir, model_dtype=model_dtype)
    ...
    if bures_backend == "auto":
        bures_backend = "gpu" if linalg_device.type == "cuda" else "cpu"
...
def build_parser():
    ...
    p.add_argument(
        ...
        help="Backend for Bures/Wasserstein geometry computation."
    )
    return p
...
def main():
    ...
    summary = run_bridge_analysis(
        ...
        bures_backend=args.bures_backend,
    )
