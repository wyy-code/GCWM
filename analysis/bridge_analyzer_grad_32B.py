...
def _build_prompt_only_batch(tokenizer, prompts: List[str], max_length: int, device: torch.device):
    enc = tokenizer(
        prompts,
        padding=True,
        truncation=True,
...
def _compute_task_gradients(
    ...
    for start in range(0, len(records), batch_size):
        ...
        # 多选题答案通常是单 token（A/B/C/D...）
        target_ids = [tokenizer(t, add_special_tokens=False).input_ids for t in targets]
        use_last_token_loss = all(len(x) == 1 for x in target_ids)

        if use_last_token_loss:
            batch = _build_prompt_only_batch(
                tokenizer=tokenizer,
                prompts=prompts,
                max_length=max_length,
                device=device,
            ...
            row_idx = torch.arange(last_pos.size(0), device=device)

            # outputs.logits: [B, T, V] -> 只取最后位置 [B, V]
            last_logits = outputs.logits[row_idx, last_pos]
            labels = torch.tensor([x[0] for x in target_ids], device=device, dtype=torch.long)

            loss = F.cross_entropy(last_logits.float(), labels, reduction="mean")
        else:
            batch = _build_causal_lm_batch(
                tokenizer,
                prompts,
                targets,
                max_length=max_length,
